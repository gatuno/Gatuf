...
class Gatuf_ModelUtils {
	...
	public static function getAssocField($model): String {
		$name = self::skipeName(strtolower($model->_a['model']) . '_'.$model->primary_key);
	 ...
	 *
	 * @param
	 *			Gatuf_Model The model.
	 * @param
	 *			array Data to bound the form (null)
	 *			
	 * @return Gatuf_Form_Model Form to create the model.
	 */
	public static function getCreateForm($model, $data = null) {
		$extra = array(
			'model' => $model
		);
		return new Gatuf_Form_Model($data, $extra, null);
	}
	...
	public static function getUpdateForm($model, $data = null) {
		...
		return new Gatuf_Form_UpdateModel($data, $extra, null);
	}
}
