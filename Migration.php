...
class Gatuf_Migration
{
    ...
    public function __construct($app=null)
    {
        if (!is_null($app)) {
            ...
            $this->apps = Gatuf::config('installed_apps');
        }
    }
    ...
    public function installApp($app, $uninstall=false)
    {
        ...
        if (!$this->dry_run) {
            ...
            if (!$uninstall) {
                ...
                if ($app != 'Gatuf') {
                    // If Gatuf we do not have the schema info table
                    // anymore
                    $this->delAppInfo($app);
                }
            }
    ...
    {
        $migrations = array();
        if (false !== ($mdir = Gatuf::fileExists($this->app.'/Migrations'))) {
