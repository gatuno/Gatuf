...
class Gatuf_SQL {
	 ...
	 */
	public function __construct($base='', $args=array()) {
		$this->db = Gatuf::db();
		if (strlen($base) > 0) {
			$this->Q($base, $args);
		}
	 ...
	 */
	public function Q($base, $args=array()) {
		$escaped = array();
		if (!is_array($args)) {
			$args = array($args);
		}
	 ...
	 */
	public function SDef($sql, $k='AND') {
		if (empty($this->ands)) {
			$this->ands = $sql->ands;
		} else {
			$othersql = $sql->gen();
	 ...
	 */
	public function keywords($string, $del=' ') {
		$keys = array();
