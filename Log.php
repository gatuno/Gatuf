...
class Gatuf_Log
{
    ...
    private static function _log($level, $message)
    {
        ...
        if (self::$level <= $level and self::$level != 10) {
            ...
            if (!Gatuf::config('log_delayed', false)) {
                self::flush();
            }
        }
    }
    ...
    {
        $writer = Gatuf::config('log_handler', 'Gatuf_Log_File');
    ...
    public static function activeAssert()
    {
        ...
        assert_options(ASSERT_CALLBACK, 'Gatuf_Log_assert');
    }
    ...
    {
        if (!isset(Gatuf_Log::$store[$key])) {
            Gatuf_Log::$store[$key] = 0;
        }
        Gatuf_Log::$store[$key] += $amount;
    }
    ...
    */
    ...
    {
        return (isset(Gatuf_Log::$store[$key])) 
            ? Gatuf_Log::$store[$key] : $value;
    }
    ...
    */
    ...
    {
        Gatuf_Log::$store['time_tracker_'.$key] = microtime(true);
    }
    ...
    */
    ...
    {
        $t = microtime(true) - Gatuf_Log::$store['time_tracker_'.$key];
        if ($total) {
            Gatuf_Log::inc('time_tracker_'.$total, $t);
        }
        return $t;
    }
}
 ...
 */
function Gatuf_Log_assert($file, $line, $code)
{
    if (Gatuf_Log::$level <= Gatuf_Log::$assert_level and 
        Gatuf_Log::$level != 10) {
        Gatuf_Log::$stack[] = array(
                                   microtime(true),
                                   Gatuf_Log::$assert_level,
                                   Gatuf_Log::$assert_mess,
                                   $file, $line, $code);
        if (!Gatuf::f('log_delayed', false)) {
            Gatuf_Log::flush();
        }
                               
    }
    Gatuf_Log::$assert_level = 6;
    Gatuf_Log::$assert_mess = null;
}
