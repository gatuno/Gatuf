...
class Gatuf_Calendar {
	 ...
	 *
	 */
	public $_simultaneous = array();
	public $_max_simultaneous = array();
	 ...
	 */
	public function getEventGroup($event) {
	 ...
	 */
	public function getEventCell($group, $inter) {
		$out = '';
	 ...
	 * @return int Spanning
	 */
	public function getEventRowSpanning($event, $inters) {
	 ...
	 */
	public function getDayNumber($start, $end) {
		Gatuf::loadFunction('Gatuf_Date_Compare');
		$diff = Gatuf_Date_Compare($start.' 00:00:00', $end.' 00:00:00');
	 ...
	 * Clean event list.
	 */
	public function cleanEventList() {
		list($start, $end) = $this->getStartEndDays();
		$this->_events = array();
		foreach ($this->events as $event) {
			$e_dstart = substr($event['start'], 0, 10);
