...
class Gatuf_L10n {
	 ...
	 */
	public function __construct($folder='', $lang='en', $domain='gatuf') {
		if ('' == $folder) {
	 ...
	 * @return bool Success
	 */
	public function loadDomain($lang='en', $domain='gatuf') {
		if ('en' == $lang) {
			return true;
		}
	...
	public function loadFile($file) {
		if (!empty($GLOBALS['_GX_locale_files'][$file])) {
			return true;
		}
		if (!file_exists($file)) {
			return false;
		}
		if (!isset($GLOBALS['_GX_locale'])) {
			$GLOBALS['_GX_locale'] = array();
		}
		// Load optimized file if available
		if ('' != Gatuf::config('tmp_folder')) {
			$phpfile = Gatuf::config('tmp_folder').'/Gatuf_L10n-'
				.str_replace(DIRECTORY_SEPARATOR, '_', substr($file, 0, -5))
				.'.php';
			if (file_exists($phpfile)
				&& (@filemtime($file) < @filemtime($phpfile))) {
				$l = include $phpfile;
				$GLOBALS['_GX_locale'] = array_merge($GLOBALS['_GX_locale'], $l);
				$GLOBALS['_GX_locale_files'][$file] = 'optimized';
				return true;
			}
		}
		$lines = file($file);
		...
		for ($i=1; $i<$count; $i++) {
			...
			if (!empty($tmp) && ';' == substr($lines[$i], 0, 1)) {
				$GLOBALS['_GX_locale'][trim(substr($lines[$i], 1))] = $tmp;
				$i++;
			}
		}
		$GLOBALS['_GX_locale_files'][$file] = true;
		return true;
	}
	 ...
	 * @return bool Success
	 */
	public function optimizeLocale($file) {
		if (!file_exists($file)) {
			return false;
		}
		$phpfile = Gatuf::config('tmp_folder').'/Gatuf_L10n-'
	 ...
	 */
	public function getAvailableLocales($domain='') {
	 ...
	 */
	public function getAcceptedLanguage($available, $accepted ='') {
		if (empty($accepted)) {
	 ...
	 */
	public function getIsoCodes($lang=false) {
		$res = array('aa' => 'Afar',
			'ab' => 'Abkhazian',
			'af' => 'Afrikaans',
			'am' => 'Amharic',
