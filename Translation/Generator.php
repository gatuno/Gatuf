 ...
 */
class Gatuf_Translation_Generator {
	public $outputdir = '';
	...
	public function generate($outputdir) {
		$dest_files = array();
		foreach (Gatuf::config('template_folders') as $folder) {
		...
		foreach ($dest_files as $src=>$dest) {
			...
			$compiler = new Gatuf_Translation_TemplateExtractor(self::real_path($src), array(''), true);
