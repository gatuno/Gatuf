 ...
 *
 */
class Gatuf_Form implements Iterator, ArrayAccess {
	/**
	...
	public function __construct($data=null, $extra=array(), $label_suffix=null) {
		if ($data !== null) {
			$this->data = $data;
			$this->is_bound = true;
		}
		...
		$this->f = new Gatuf_Form_FieldProxy($this);
	}
	...
	public function render_top_errors() {
		...
		array_walk($top_errors, 'Gatuf_Form_htmlspecialcharsArray');
		return new Gatuf_Template_SafeString(Gatuf_Form_renderErrorsAsHTML($top_errors), true);
	}
	...
	protected function htmlOutput(
		...
		$hidden_fields = array();
		foreach ($this->fields as $name=>$field) {
			$bf = new Gatuf_Form_BoundField($this, $field, $name);
			$bf_errors = $bf->errors;
			array_walk($bf_errors, 'Gatuf_Form_htmlspecialcharsArray');
			if ($field->widget->is_hidden) {
				foreach ($bf_errors as $_e) {
					$top_errors[] = sprintf(
						'(Hidden field %1$s) %2$s',
						$name,
						$_e
					);
				}
				...
				if ($errors_on_separate_row and count($bf_errors)) {
					$output[] = sprintf($error_row, Gatuf_Form_renderErrorsAsHTML($bf_errors));
				}
				...
				$errors = '';
				if (!$errors_on_separate_row and count($bf_errors)) {
					$errors = Gatuf_Form_renderErrorsAsHTML($bf_errors);
				}
				$output[] = sprintf(
					$normal_row,
					$errors,
		...
		if (count($top_errors)) {
			$errors = sprintf(
				$error_row,
				Gatuf_Form_renderErrorsAsHTML($top_errors)
			);
	...
	public function field($key) {
		return new Gatuf_Form_BoundField($this, $this->fields[$key], $key);
	}
	...
	public function current() {
		...
		return new Gatuf_Form_BoundField($this, $field, $name);
	}
...
function Gatuf_Form_htmlspecialcharsArray(&$item, $key) {
	$item = htmlspecialchars($item, ENT_COMPAT, 'UTF-8');
}

function Gatuf_Form_renderErrorsAsHTML($errors) {
	$tmp = array();
