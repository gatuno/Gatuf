...
class Gatuf_Precondition {
	 ...
	 * @return mixed
	 */
	public static function staffRequired($request) {
	 ...
	 * @return mixed
	 */
	public static function adminRequired($request) {
		$res = Gatuf_Precondition::loginRequired($request);
		if (true !== $res) {
			return $res;
		}
	 ...
	 * @return mixed
	 */
	public static function sslRequired($request) {
		if (empty($_SERVER['HTTPS']) or $_SERVER['HTTPS'] == 'off') {
			return new Gatuf_HTTP_Response_Redirect('https://'.$request->http_host.$request->uri);
		}
		return true;
	}
}
