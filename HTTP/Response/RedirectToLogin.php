 ...
 * access a page.
 */
class Gatuf_HTTP_Response_RedirectToLogin extends Gatuf_HTTP_Response {
	/**
	 ...
	 */
	public function __construct($request, $loginurl=null) {
		if ($loginurl !== null) {
			...
			$url = Gatuf_HTTP_URL_urlForView(
				Gatuf::config('login_view', 'login_view'),
				array(),
				array('_redirect_after' => $request->uri),
				false
			);
			$encoded = Gatuf_HTTP_URL_urlForView(
				Gatuf::config('login_view', 'login_view'),
				array(),
				array('_redirect_after' => $request->uri)
			);
