...
class Gatuf_HTTP_Response_ServerError extends Gatuf_HTTP_Response {
	public function __construct($exception, $mimetype=null) {
		$content = '';
		$admins = Gatuf::config('admins', array());
		if (count($admins) > 0) {
			// Get a nice stack trace and send it by emails.
			$stack = Gatuf_HTTP_Response_ServerError_Pretty($exception);
		...
		try {
			$context = new Gatuf_Template_Context(array('message' => $exception->getMessage()));
			$tmpl = new Gatuf_Template('500.html');
