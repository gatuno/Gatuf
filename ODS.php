...
class Gatuf_ODS {
	...
	public function addStringCell ($sheet, $row, $col, $string) {
		...
		if ($this->max_col[$sheet] < $col) $this->max_col[$sheet] = $col;
		if ($this->max_row[$sheet] < $row) $this->max_row[$sheet] = $row;
	...
	public function construir_contenido () {
		...
		if (count ($this->hojas) == 0) throw new Exception ('El libro no tiene hojas');
