...
class Gatuf_Paginator {
	 ...
	 */
	public function configure($list_display, $search_fields=array(), $sort_fields=array()) {
	...
	public function setFromRequest($request) {
		if (isset($request->REQUEST['s'])) {
			$this->search_string = $request->REQUEST['s'];
		}
		if (isset($request->REQUEST['p'])) {
			$this->current_page = (int) $request->REQUEST['p'];
			$this->current_page = max(1, $this->current_page);
		}
		if (isset($request->REQUEST['sk'])
			and in_array($request->REQUEST['sk'], $this->sort_fields)) {
			$this->sort_order[0] = $request->REQUEST['sk'];
			$this->sort_order[1] = 'ASC';
			if (isset($request->REQUEST['so'])
				and ($request->REQUEST['so'] == 'd')) {
				$this->sort_order[1] = 'DESC';
			}
		}
		if (isset($request->REQUEST['fk'])
			and in_array($request->REQUEST['fk'], $this->list_filters)
			and isset($request->REQUEST['fv'])) {
			// We add a forced where query
			$sql = new Gatuf_SQL(
				$request->REQUEST['fk'].'=%s',
				$request->REQUEST['fv']
			);
			...
			$this->active_list_filter = array($request->REQUEST['fk'],
				$request->REQUEST['fv']);
		}
	}
	...
	public function render($id='') {
		...
		$out .= $this->body();
		$out .= '</table>'."\n";
		return new Gatuf_Template_SafeString($out, true);
	}
	...
	public function render_array() {
		...
		foreach ($items as $item) {
			...
			if (!empty($this->list_display)) {
				...
				throw new Exception("Paginador: Oops, no se han especificado columnas a desplegar");
			}
			$out[] = $idata;
		}
		return $out;
	 ...
	 */
	public function getEditAction($text, $item) {
		$edit_action = $this->edit_action;
		if (!empty($edit_action)) {
			if (!is_array($edit_action)) {
				throw new Exception('Paginador: Oops, los modelos no trabajan con un ID');
			...
			$url = Gatuf_HTTP_URL_urlForView($view, $params);
		...
		if (empty($this->list_display)) {
			...
			foreach ($this->list_display as $key=>$col) {
				if (is_array($col)) {
					$field = $col[0];
					$name = $col[2];
					Gatuf::loadFunction($col[1]);
				} else {
					$name = $col;
					$field = $key;
				}
				if (!$this->sort_link_title) {
					$out .= '<th><span class="px-header-title">'.Gatuf_esc(ucfirst($name)).'</span>'.$this->headerSortLinks($field).'</th>';
				} else {
					$out .= '<th><span class="px-header-title">'.$this->headerSortLinks($field, Gatuf_esc(ucfirst($name))).'</span></th>';
				}
			}
			$out .= '</tr>'."\n";
			return $out;
	 ...
	 */
	public function headerSortLinks($field, $title='') {
	 ...
	 * @return string Url.
	 */
	public function getUrl($get_params=array(), $encoded=true) {
		// Default values
		$params = array();
		$by_name = false;
		$view = '';
 ...
 */
function Gatuf_Paginator_ToString($field, $item) {
	return Gatuf_esc($item);
}

/**
 * Returns the item referenced as foreign key as a string.
 */
function Gatuf_Paginator_FkToString($field, $item) {
	$method = 'get_'.$field;
	$fk = $item->$method();
	return Gatuf_esc($fk);
}
