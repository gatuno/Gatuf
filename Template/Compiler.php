...
class Gatuf_Template_Compiler {
	...
	protected $_modifier = array('upper' => 'strtoupper',
		...
		'debug' => 'print_r',
		'dump' => 'Gatuf_Template_varExport',
		'fulldebug' => 'var_export',
		'nl2br' => 'Gatuf_Template_nl2br',
		'trim' => 'trim',
		'ltrim' => 'ltrim',
		'rtrim' => 'rtrim',
	...
	public function getCompiledTemplate() {
		...
		if (count($this->_usedModifiers)) {
			...
			foreach ($this->_usedModifiers as $modifier) {
				$code[] = 'Gatuf::loadFunction(\''.$modifier.'\'); ';
			}
			$result = '<?php '.implode("\n", $code).'?>'.$result;
		}
		// Clean the output
	...
	public function _callback($matches) {
		...
		if (in_array($firstcar, array('$', '\'', '"'))) {
			if ('blocktrans' !== end($this->_blockStack)) {
				return '<?php Gatuf_Template_safeEcho('.$this->_parseVariable($tag).'); ?>';
			} else {
				$tok = explode('|', $tag);
	...
	public function _parseFunction($name, $args) {
		...
		case '/blocktrans':
			...
			if ($this->_transPlural) {
				...
				$res .= 'Gatuf_Translation::sprintf(_n($_b_t_s, $_b_t_p, $_b_t_c), array(';
				$_tmp = array();
		...
		case 'include':
			...
			$_comp = new Gatuf_Template_Compiler($argfct, $this->templateFolders);
			$res = $_comp->compile();
			...
			if (isset($this->_extraTags[$name])) {
				if (false == $_end) {
					if (method_exists($this->_extraTags[$name], 'start')) {
						$res .= '$_extra_tag = Gatuf::factory(\''.$this->_allowedTags[$name].'\', $t); $_extra_tag->start('.$argfct.'); ';
					}
					...
					if (method_exists($this->_extraTags[$name], 'end')) {
						$res .= '$_extra_tag = Gatuf::factory(\''.$this->_allowedTags[$name].'\', $t); $_extra_tag->end('.$argfct.'); ';
					}
