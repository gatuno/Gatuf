...
class Gatuf_DB_PostgreSQL {
	...
	function __construct($user, $pwd, $server, $dbname, $pfx = '', $debug = false) {
		...
		$this->type_cast['Gatuf_DB_Field_Boolean'] = array(
			'Gatuf_DB_PostgreSQL_BooleanFromDb',
			'Gatuf_DB_BooleanToDb'
		);
		$this->type_cast['Gatuf_DB_Field_Compressed'] = array(
		...
		if (! $this->con_id) {
			throw new Gatuf_Exception($this->getError());
		}
	}
	...
	public function getAffectedRows() {
		return (int) pg_affected_rows($this->cur);
	}
	...
	function setSearchPath($search_path = 'public') {
		if (preg_match('/[^\w\s\,]/', $search_path)) {
			throw new Gatuf_Exception('The search path: "' . $search_path . '" is not valid.');
		}
...
function Gatuf_DB_PostgreSQL_BooleanFromDb($val)
{
	if (! $val) {
		return false;
	}
...
function Gatuf_DB_PostgreSQL_CompressedToDb($val, $con)
{
	if (is_null($val)) {
		return 'NULL';
	}
...
function Gatuf_DB_PostgreSQL_CompressedFromDb($val)
{
