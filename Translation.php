...
class Gatuf_Translation {
	...
	public static function loadSetLocale($lang) {
		...
		$GLOBALS['_GX_locale'][$lang] = array();
		foreach (Gatuf::config('installed_apps') as $app) {
			if (false != ($pofile=Gatuf::fileExists($app.'/locale/'.$lang.'/'.strtolower($app).'.po'))) {
				$GLOBALS['_GX_locale'][$lang] += Gatuf_Translation::readPoFile($pofile);
			}
		}
	}

	public static function getLocale() {
		return $GLOBALS['_GX_current_locale'];
	}
